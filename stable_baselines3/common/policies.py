...
class BaseModel(nn.Module):
    ...
    def __init__(
        ...
        num_classes: List[Optional[int]] = None,
...
class BasePolicy(BaseModel, ABC):
    ...
    def predict(
        ...
        # print('[BasePolicy]', obs_tensor['images'].shape, obs_tensor['last_action'].shape)
...
class ActorCriticPolicy(BasePolicy):
    ...
    def _build(self, lr_schedule: Schedule) -> None:
        ...
        # self.concept_net = nn.Linear(latent_dim_pi, self.concept_dim) # no activation function
        self.concept_net = FlexibleMultiTaskNetwork(self.features_dim, self.task_types, self.num_classes, self.focal_loss)

        self._build_mlp_extractor() # concept_dim -> 64
        latent_dim_pi = self.mlp_extractor.latent_dim_pi
        # feature -> FlexibleMultiTaskNetwork -> mlp_extractor (both) -> action_net & value_net
        ...
        #     if 'features_extractor' not in name and 'concept_net' not in name:
        #         print(name, param.shape)
        self.optimizer_exclude_concept = self.optimizer_class(
            [
                param for name, param in self.named_parameters()
                if 'features_extractor' not in name and 'concept_net' not in name # since shared features extractor! should check carefully if as desired!
            ], 
    ...
    def forward(self, obs: th.Tensor, concepts: Optional[th.Tensor] = None, deterministic: bool = False) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        """
        ...
        """
        # print('[Policy forward]', obs['images'].shape, obs['last_action'].shape)
        ...
        if concepts is None:
            concepts = self.concept_net(pi_features)
        latent_pi, latent_vf = self.mlp_extractor(concepts, vf_features)
    ...
    def _get_action_dist_from_latent(self, latent_pi: th.Tensor) -> Distribution:
        ...
        """
        # print('concepts', latent_pi.shape)
    ...
    def predict_concepts(self, obs: PyTorchObs) -> th.Tensor:
        """
        Get the concepts according to the current policy given the observations.

        :param obs: Observation
        :return: the estimated concepts.
        """
        features = self.extract_features(obs)
        if self.share_features_extractor:
            pi_features = features
    ...
    def evaluate_actions(self, obs: PyTorchObs, actions: th.Tensor, concepts: Optional[th.Tensor] = None) -> Tuple[th.Tensor, th.Tensor, Optional[th.Tensor]]:
        """
        ...
        values = self.value_net(latent_vf)
        return concepts, values, log_prob, entropy
    ...
    def get_distribution(self, obs: PyTorchObs) -> Distribution:
        ...
        # print('[get_distribution] latent_pi concepts', latent_pi.shape, concepts.shape)
...
class CustomConceptNet(nn.Module): # only observation -> concept, used for active learning
    def __init__(self, policy: ActorCriticPolicy, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self.feature_extractor = policy.make_features_extractor()
        self.concept_net = FlexibleMultiTaskNetwork(policy.features_dim, policy.task_types, policy.num_classes, policy.focal_loss)

    def forward(self, obs: th.Tensor) -> th.Tensor:
        features = self.feature_extractor(obs)
        return self.concept_net(features)
    
    def get_logits(self, obs: th.Tensor) -> th.Tensor:
        features = self.feature_extractor(obs)
        return self.concept_net.get_logits(features)