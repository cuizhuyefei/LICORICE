...
class BaseAlgorithm(ABC):
    ...
    def __init__(
        ...
        # print('BaseAlgorithm -- policy_kwargs', self.policy_kwargs)
        ...
        if env is not None:
            ...
            # self.concept_dim = env.envs[0].concept_dim # if defined in the environment wrapper
    ...
    def _update_current_progress_remaining(self, num_timesteps: int, total_timesteps: int) -> None:
        ...
        """
        self._current_progress_remaining = 1.0 - min(float(num_timesteps) / float(total_timesteps), 1.0)
    ...
    def _setup_learn(
        ...
        if reset_num_timesteps or self._last_obs is None:
            ...
            self._last_con = np.stack(self.env.env_method("get_concept"))
            # self._last_con = np.stack([con for con in each_con]) # strange!
            # self._last_con = np.stack(each_con) # strange!
            # print('DEBUG', self._last_con, type(self._last_con))
            # print('BaseAlgorithm -- env.reset()', self._last_obs.shape, self._last_con.shape) # , info
    ...
    def save(
        ...
        # print('trying to save:', data.keys())
