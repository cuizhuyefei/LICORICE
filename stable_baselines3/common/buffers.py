...
class RolloutBuffer(BaseBuffer):
    ...
    def __init__(
        ...
        super().__init__(buffer_size, observation_space, concept_dim, action_space, device, n_envs=n_envs)
    ...
    def reset(self) -> None:
        ...
        self.concepts = np.zeros((self.buffer_size, self.n_envs, self.concept_dim), dtype=np.float32)
    ...
    def add(
        ...
        self.observations[self.pos] = np.array(obs)
        self.concepts[self.pos] = np.array(concept)
        self.actions[self.pos] = np.array(action)
        self.rewards[self.pos] = np.array(reward)
...
class DictRolloutBuffer(RolloutBuffer):
    ...
    def __init__(
        ...
        super(RolloutBuffer, self).__init__(buffer_size, observation_space, concept_dim, action_space, device, n_envs=n_envs)
    ...
    def get(  # type: ignore[override]
        ...
        if not self.generator_ready:
            ...
            _tensor_names = ["concepts", "actions", "values", "log_probs", "advantages", "returns"]
    ...
    def _get_samples(  # type: ignore[override]
        ...
        return DictRolloutBufferSamples(
            ...
            concepts=self.to_torch(self.concepts[batch_inds]),
